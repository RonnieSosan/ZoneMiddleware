...
namespace Blend.DefaultImplementation.ProfileService
{
    public class DefaultTokenManager : ITokenManager
    {
        ...
        public UserProfileResponse NonTransactionalTokenAuthentication(AuthenticationData authRequest)
        {
            Logger.LogInfo("UserProfileService.NonTransactionalTokenAuthentication, input", authRequest);
                    ...
                    if (verifingAccount != null)
                    {
                        ...
                        if (verifingAccount.TokenExpDate >= System.DateTime.Now)
                        {
                            ...
                            catch (Exception ex)
                            {
                                ...
                                {
                                    ResponseCode = "06",
                                    ResponseDescription = "Error while authenticating data: " + ex.Message,
                                };
                            }
                        }
                        else
        ...
        public UserProfileResponse TransactionalTokenAuthentication(AuthenticationData authRequest)
        {
            Logger.LogInfo("UserProfileService.TransactionalTokenAuthentication, input", authRequest);
            UserProfileResponse response = null;
            try
            {
