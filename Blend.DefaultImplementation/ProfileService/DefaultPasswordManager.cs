...
namespace Blend.DefaultImplementation.ProfileService
{
    public class DefaultPasswordManager : IPasswordManager
    {
        ...
        public DefaultPasswordManager(IMailService mailService, ISMSSender smsSender)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(passwordRequest.AuthToken))
                {
                    ...
                    if (passwordRequest.MailRequest != null)
                    {
                        ...
                        _mailService.SendMailToCustomer(passwordRequest.MailRequest);
                    }
                ...
                if (verifingAccount != null)
                {
                    ...
                    if (userOtp != null)
                    {
                        ...
                        if (forgotPasswordRequest.MailRequest != null)
                        {
                            ...
                            _mailService.SendMailToCustomer(forgotPasswordRequest.MailRequest);

                            var sms = string.Format("Verification Code: {0}", userOTP);

                            _smsSender.Send(new SMSRequest { customer_id = forgotPasswordRequest.CustomerID, Message = sms });
                        }
