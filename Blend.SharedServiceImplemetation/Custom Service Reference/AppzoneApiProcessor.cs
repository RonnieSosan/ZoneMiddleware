...
{
    public class AppzoneApiProcessor
    {
        public string Url = System.Configuration.ConfigurationManager.AppSettings.Get("ApppzoneApiUrl");
        private static HttpClient client = null;

        public AppzoneApiProcessor(string Platform, string Service, string Operation)
        {
        ...
        public string CallService <T>(string RequestMessage)
        {
            ...
            try {
                ...
                Logger.LogInfo("AppzoneApiProcessor.CallService, ServiceTimeOut", System.Configuration.ConfigurationManager.AppSettings.Get("ServiceTimeOut"));
                using (client = new HttpClient())
                {
                    client.Timeout = TimeSpan.FromSeconds(Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings.Get("ServiceTimeOut")));
